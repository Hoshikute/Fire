...
namespace Procedure
{
    public class ProcedureInitResources : ProcedureBase
    {
        ...
        protected override void OnEnter(ProcedureOwner procedureOwner)
        {
            ...
            InitResourcesAsync(procedureOwner).Forget();
        }
        ...
        protected override void OnUpdate(ProcedureOwner procedureOwner, float elapseSeconds, float realElapseSeconds)
        {
            ...
            if (ResourceModule.PlayMode == EPlayMode.HostPlayMode || ResourceModule.PlayMode == EPlayMode.WebPlayMode)
            {
                //线上最新版本operation.PackageVersion
                Log.Debug($"Updated package Version : from {ResourceModule.GetPackageVersion()} to {ResourceModule.PackageVersion}");
                //注意：保存资源版本号作为下次默认启动的版本!
                // 如果当前是WebGL或者是边玩边下载直接进入预加载阶段。
                if (ResourceModule.PlayMode == EPlayMode.WebPlayMode ||
                    ResourceModule.UpdatableWhilePlaying)
                {
                    // 边玩边下载还可以拓展首包支持。
        ...
        private async UniTaskVoid InitResourcesAsync(ProcedureOwner procedureOwner)
        {
            Log.Info("更新资源清单！！！");
            LauncherMgr.ShowUI<LoadUpdateUI>($"更新清单文件...");

            // 1. 获取资源清单的版本信息
            var operation1 = ResourceModule.RequestPackageVersionAsync();
            await operation1;
            ...
            ResourceModule.PackageVersion = packageVersion;
            ...
            // 2. 传入的版本信息更新资源清单
            var operation2 = ResourceModule.UpdatePackageManifestAsync(packageVersion);
            await operation2;
        ...
        private void OnInitResourcesError(ProcedureOwner procedureOwner, string message)
        {
            ...
            LauncherMgr.ShowMessageBox($"初始化资源失败！点击确认重试 \n <color=#FF0000>{message}</color>"
                , () => { InitResourcesAsync(procedureOwner).Forget(); }, Application.Quit);
        }

        private bool IsNeedUpdate()
        {
            // 如果不能联网且当前游戏非强制(不更新可以进入游戏。)
            if (Settings.UpdateSetting.UpdateStyle == UpdateStyle.Optional && !ResourceModule.UpdatableWhilePlaying)
            {
                // 获取上次成功记录的版本
                ...
                if (string.IsNullOrEmpty(packageVersion))
                {
                    ...
                    LauncherMgr.ShowMessageBox("没有找到本地版本记录，需要更新资源！",
                        () => { InitResourcesAsync(_procedureOwner).Forget(); },
                        Application.Quit);
                    return false;
                }

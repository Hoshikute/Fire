...
namespace EGamePlay.Combat
{
    ...
    public sealed class MotionComponent : Component
    {
        ...
        {
            get => GetEntity<CombatEntity>().Position;
            set => GetEntity<CombatEntity>().Position = value;
        }

        public Quaternion Rotation
        {
            get => GetEntity<CombatEntity>().Rotation;
            set => GetEntity<CombatEntity>().Rotation = value;
        }
        ...
        private void MoveFinish()
        {
            ...
            if (combatContext == null || combatContext.HeroEntities == null || combatContext.HeroEntities.Count == 0)
            {
                return;
            }

            CombatEntity heroEntity = null;
            if (combatContext.HeroEntities.TryGetValue(1, out var seatOneHero))
            {
                heroEntity = seatOneHero;
            }
            else
            {
                heroEntity = combatContext.HeroEntities.Values.FirstOrDefault();
            }
